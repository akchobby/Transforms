...
class Quaternion(Rotations):
    ...
    def to_euler(self, deg=False, order='zyx'):
        ...
        if order == "zyx":
            ...
            pitch =  math.asin(max(min(r[0][2],1), -1))
            roll =  math.atan2(-r[1][2],r[2][2]) if math.fabs(r[0][2]) < 1- np.finfo(float).eps else math.atan2(r[2][1],r[1][1])
            yaw = math.atan2(-r[0][1],r[0][0]) if math.fabs(r[0][2]) < 1- np.finfo(float).eps else 0.0
